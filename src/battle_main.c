...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    TRAINER_CLASS(HIKER, "Hiker", 8),
    TRAINER_CLASS(TEAM_AQUA, "Team Aqua"),
    TRAINER_CLASS(PKMN_BREEDER, "{PKMN} Breeder", 8, B_TRAINER_CLASS_POKE_BALLS >= GEN_8 ? ITEM_HEAL_BALL : ITEM_FRIEND_BALL),
    TRAINER_CLASS(COOLTRAINER, "Ace Trainer", 10, ITEM_ULTRA_BALL),
    TRAINER_CLASS(BIRD_KEEPER, "Bird Keeper", 8),
    TRAINER_CLASS(COLLECTOR, "Collector", 8, ITEM_PREMIER_BALL),
    TRAINER_CLASS(SWIMMER_M, "Swimmer♂", 6, ITEM_DIVE_BALL),
    ...
    TRAINER_CLASS(HEX_MANIAC, "Hex Maniac", 8),
    TRAINER_CLASS(AROMA_LADY, "Aroma Lady", 8),
    TRAINER_CLASS(RUIN_MANIAC, "Ruin Maniac", 8),
    TRAINER_CLASS(INTERVIEWER, "Interviewer", 8),
    TRAINER_CLASS(TUBER_F, "Tuber", 1),
    TRAINER_CLASS(TUBER_M, "Tuber", 1),
    TRAINER_CLASS(LADY, "Lady", 15),
    TRAINER_CLASS(BEAUTY, "Beauty", 10),
    TRAINER_CLASS(RICH_BOY, "Rich Boy", 15),
    TRAINER_CLASS(POKEMANIAC, "Pokémaniac", 8),
    TRAINER_CLASS(GUITARIST, "Guitarist", 8),
    TRAINER_CLASS(KINDLER, "Kindler", 8),
    TRAINER_CLASS(CAMPER, "Camper", 6),
    TRAINER_CLASS(PICNICKER, "Picnicker", 6),
    TRAINER_CLASS(BUG_MANIAC, "Bug Maniac", 8),
    ...
    TRAINER_CLASS(POKEFAN, "Pokéfan", 8),
    TRAINER_CLASS(YOUNGSTER, "Youngster", 4),
    TRAINER_CLASS(CHAMPION, "Champion", 50),
    TRAINER_CLASS(FISHERMAN, "Fisherman", 8, B_TRAINER_CLASS_POKE_BALLS >= GEN_8 ? ITEM_DIVE_BALL : ITEM_LURE_BALL),
    TRAINER_CLASS(TRIATHLETE, "Triathlete", 8),
    TRAINER_CLASS(DRAGON_TAMER, "Dragon Tamer", 10),
    TRAINER_CLASS(NINJA_BOY, "Ninja Boy", 4),
    TRAINER_CLASS(BATTLE_GIRL, "Battle Girl", 8),
    TRAINER_CLASS(PARASOL_LADY, "Parasol Lady", 8),
    TRAINER_CLASS(SWIMMER_F, "Swimmer♀", 6, ITEM_DIVE_BALL),
    TRAINER_CLASS(TWINS, "Twins", 2),
    TRAINER_CLASS(SAILOR, "Sailor", 8),
    TRAINER_CLASS(COOLTRAINER_2, "Ace Trainer", 10, ITEM_ULTRA_BALL),
    TRAINER_CLASS(MAGMA_ADMIN, "Magma Admin", 10),
    TRAINER_CLASS(RIVAL, "{PKMN} Trainer", 15),
    TRAINER_CLASS(BUG_CATCHER, "Bug Catcher", 4),
    TRAINER_CLASS(PKMN_RANGER, "{PKMN} Ranger", 10),
    ...
    TRAINER_CLASS(OLD_COUPLE, "Old Couple", 15),
    TRAINER_CLASS(SIS_AND_BRO, "Sis and Bro", 8),
    ...
    TRAINER_CLASS(SCIENTIST, "Scientist", 8, ITEM_REPEAT_BALL),
    TRAINER_CLASS(WAITER, "Waiter", 8, ITEM_LUXURY_BALL),
    TRAINER_CLASS(WAITRESS, "Waitress", 8, ITEM_LUXURY_BALL),
    TRAINER_CLASS(FAIRY_TALE_GIRL, "FaeTaleGirl", 4, ITEM_FRIEND_BALL),
    TRAINER_CLASS(SOCIALITE, "Socialite", 20, ITEM_LUXURY_BALL),
    TRAINER_CLASS(BACKPACKER, "Backpacker", 8, ITEM_GREAT_BALL),
    TRAINER_CLASS(WORKER, "Worker", 8, ITEM_HEAVY_BALL),
    TRAINER_CLASS(ROUGHNECK, "Roughneck", 8, ITEM_NEST_BALL),
    TRAINER_CLASS(MIRAGE, "Mirage", 8, ITEM_MASTER_BALL),
};
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            if (partyData[k].nature > 0)
                CreateMonWithGenderNatureLetter(&party[i], partyData[k].species, partyData[k].lvl, fixedIV, gender, partyData[k].nature, 0, partyData[k].isShiny ? OT_ID_SHINY : OT_ID_RANDOM_NO_SHINY);
            else
                CreateMonWithGenderNatureLetter(&party[i], partyData[k].species, partyData[k].lvl, fixedIV, gender, NATURE_SERIOUS, 0, partyData[k].isShiny ? OT_ID_SHINY : OT_ID_RANDOM_NO_SHINY);
            ...
            if (partyData[k].status > 0)
            {
                ...
                else if (partyData[k].status == TRAINER_MON_SLEEP)
                    status = STATUS1_SLEEP;
                else if (partyData[k].status == TRAINER_MON_FROSTBITE)
            ...
            {
                SetMonData(&party[i], MON_DATA_HP_IV, &(partyData[k].ivs[0]));
                SetMonData(&party[i], MON_DATA_ATK_IV, &(partyData[k].ivs[1]));
                SetMonData(&party[i], MON_DATA_DEF_IV, &(partyData[k].ivs[2]));
                SetMonData(&party[i], MON_DATA_SPATK_IV, &(partyData[k].ivs[3]));
            ...
            if (partyData[k].evs[0] > 0)
            {
                ...
                SetMonData(&party[i], MON_DATA_ATK_EV, &(partyData[k].evs[1]));
                SetMonData(&party[i], MON_DATA_DEF_EV, &(partyData[k].evs[2]));
                SetMonData(&party[i], MON_DATA_SPATK_EV, &(partyData[k].evs[3]));
                SetMonData(&party[i], MON_DATA_SPDEF_EV, &(partyData[k].evs[4]));
            ...
            if (partyData[k].nickname != NULL)
            {
                SetMonData(&party[i], MON_DATA_NICKNAME, partyData[k].nickname);
            }
            if (partyData[k].isShiny)
            {
                u32 data = TRUE;
                SetMonData(&party[i], MON_DATA_IS_SHINY, &data);
            }
            if (partyData[k].dynamaxLevel > 0)
            {
                u32 data = partyData[k].dynamaxLevel;
                SetMonData(&party[i], MON_DATA_DYNAMAX_LEVEL, &data);
            }
            if (partyData[k].gigantamaxFactor)
            {
                u32 data = partyData[k].gigantamaxFactor;
                SetMonData(&party[i], MON_DATA_GIGANTAMAX_FACTOR, &data);
            }
            if (partyData[k].teraType > 0)
            {
                u32 data = partyData[k].teraType;
...
void SwitchInClearSetData(u32 battler)
{
    ...
    gDisableStructs[battler].rolloutTimer = 5;
...
static void HandleEndTurn_BattleWon(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && !(gBattleTypeFlags & BATTLE_TYPE_LINK) && gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_PKMN_TRAINER_1)
    {
