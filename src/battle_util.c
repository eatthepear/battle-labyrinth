...
static enum MoveCanceler CancelerPPDeduction(struct BattleContext *ctx)
{
    ...
    if ((GetCurrentDifficultyLevel() >= DIFFICULTY_BRUTAL) && (GetBattlerSide(ctx->battlerAtk) == B_SIDE_OPPONENT))
        ppToDeduct = 0;
        ...
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_HARVEST:
                if ((IsBattlerWeatherAffected(battler, B_WEATHER_SUN) || RandomPercentage(RNG_HARVEST, 50))
                 ...
                 && ((GetCurrentDifficultyLevel() == DIFFICULTY_EASY) || (gBattleTypeFlags & BATTLE_TYPE_TRAINER) || FlagGet(FLAG_FORCE_BATTLE_ANIM_ON)))
                {
...
{
    if ((GetCurrentDifficultyLevel() >= DIFFICULTY_HARD) && ((gBattleTypeFlags & BATTLE_TYPE_TRAINER) || FlagGet(FLAG_FORCE_BATTLE_ANIM_ON)))
        return FALSE;
