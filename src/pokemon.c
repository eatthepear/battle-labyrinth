...
#include "data/pokemon/level_up_learnsets/pbl.h" // PBL
...
u16 GetBattleBGM(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        ...
        default:
            if (gTrainers[0][TRAINER_BATTLE_PARAM.opponentA].mugshotEnabled == TRUE) {
...
static const u16 sSpeciesToRandomize[] =
{
    ...
    SPECIES_COTTONEE,
    SPECIES_PETILIL,
    // SPECIES_BASCULIN_RED_STRIPED, skip repeat
    // SPECIES_BASCULIN_BLUE_STRIPED, skip repeat
    SPECIES_BASCULIN_WHITE_STRIPED,
    SPECIES_SANDILE,
    SPECIES_DARUMAKA,
    SPECIES_DARUMAKA_GALAR,
...
u16 GetRandomSpecies(u8 wildMonLevel)
{
    ...
    if (evolutions != NULL) {
        for (i = 0; evolutions[i].method != EVOLUTIONS_END; i++)
        {
            if (evolutions[i].method == EVO_LEVEL && evolutions[i].param <= wildMonLevel)
            {
