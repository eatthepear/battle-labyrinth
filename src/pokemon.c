            ...
            while (effectFlags != 0)
            {
                if (effectFlags & 1)
                {
                    ...
                    case 2: // ITEM4_HEAL_HP
                    {
                        ...
                        if (FlagGet(FLAG_SETTINGS_NUZLOCKE) && (ENABLED_ON_RELEASE || item == ITEM_MAX_REVIVE))
                        {
                            itemEffectParam++;
                            break;
                        }
...
u16 GetBattleBGM(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        ...
        case TRAINER_CLASS_RAINBOW_ROCKET:
...
static metloc_u8_t ResolveMetLcation(metloc_u8_t metLocationTemplate)
{
    return metLocationTemplate;
}
...
void CreateMonFromTemplate(struct Pokemon *mon, const struct PokemonTemplate *monTemplate)
{
    ...
    metloc_u8_t metLocation = ResolveMetLcation(monTemplate->metLocation);
