...
void FieldGetPlayerInput(struct FieldInput *input, u16 newKeys, u16 heldKeys)
{
    ...
    {
        if ((heldKeys & DEBUG_OVERWORLD_HELD_KEYS) && input->DEBUG_OVERWORLD_TRIGGER_EVENT && FlagGet(FLAG_IS_DEBUGGING_SAVEFILE))
        {
...
static const u8 *GetInteractedBackgroundEventScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    case BG_EVENT_HIDDEN_ITEM:
        ...
        if (FlagGet(gSpecialVar_0x8004) == TRUE ||
            ((FlagGet(FLAG_SETTINGS_BRUTAL) || FlagGet(FLAG_SETTINGS_INDEPENDENT)) && (gSpecialVar_0x8004 != FLAG_HIDDEN_ITEM_ZONE10A_YELLOW_SHARD_1) && (gSpecialVar_0x8004 != FLAG_HIDDEN_ITEM_ZONE10A_YELLOW_SHARD_2) && (gSpecialVar_0x8004 != FLAG_HIDDEN_ITEM_ZONE10A_YELLOW_SHARD_3)))
            return NULL;
...
static const u8 *GetInteractedWaterScript(struct MapPosition *unused1, u8 metatileBehavior, u8 direction)
{
    if (IsPlayerFacingSurfableFishableWater() == TRUE)
        return EventScript_UseSurf_PBL;

    if (MetatileBehavior_IsWaterfall(metatileBehavior) == TRUE)
    {
        if (FlagGet(FLAG_SYS_ENABLE_WATERFALL) == TRUE && IsPlayerSurfingNorth() == TRUE)
            return EventScript_UseWaterfall_PBL;
        else
            return EventScript_CannotUseWaterfall_PBL;
    }
    return NULL;
}
...
static bool32 TrySetupDiveDownScript(void)
{
    if (TrySetDiveWarp() == 2)
    {
        ScriptContext_SetupScript(EventScript_DiveSpot_PBL);
        return TRUE;
    }
    return FALSE;
}
