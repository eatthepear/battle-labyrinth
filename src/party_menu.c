...
{
    InitWindows(sSinglePartyMenuWindowTemplate_Equal); // Custom party menu
    LoadPartyMenuWindows();
}
...
static void SwitchSelectedMons(u8 taskId)
{
    ...
    else
    {
        ...
        tSlot1Offset = 0;
        if (tSlot1BaseBlock == 0x63 || tSlot1BaseBlock == 0xEF || tSlot1BaseBlock == 0x17B) //Custom party menu
            tSlot1SlideDir = -1;
        ...
        tSlot2Offset = 0;
        if (tSlot2BaseBlock == 0x63 || tSlot2BaseBlock == 0xEF || tSlot2BaseBlock == 0x17B) //Custom party menu
            tSlot2SlideDir = -1;
...
static void Task_LearnedMove(u8 taskId)
{
    ...
    else
    {
        ...
        u32 maxPP = GetMovePP(gPartyMenu.data1);
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    if (targetSpecies != SPECIES_NONE)
    {
        GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE, NULL, &canStopEvo, DO_EVO);
        FreePartyPointers();
        if (GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_RareCandy && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1) && (GetItemPocket(gSpecialVar_ItemId) != POCKET_KEY_ITEMS))
...
static void Task_PokevialLoop(u8 taskId)
{
    ...
    if (sPartyMenuInternal->tHadEffect)
    {
        ...
        DisplayPartyMenuMessage(COMPOUND_STRING("Your Pokémon were restored\nto full health.{PAUSE_UNTIL_PRESS}"), FALSE);
    }
    else
        DisplayPartyMenuMessage(gText_WontHaveEffect, TRUE);
